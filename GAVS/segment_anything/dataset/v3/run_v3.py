...
def run(model, device='cuda:1', ckpt_dir='./none', data_ver=DATA):
    ...
    # train_dataset = AVS('v3_1_shot/test', feature_dir, device=device, model=model.model_v, audio_from=audio_from)  # 1-shot test
    # train_dataset = AVS('v3_3_shot/train', feature_dir, device=device, model=model.model_v, audio_from=audio_from)  # 3-shot train
    # train_dataset = AVS('v3_3_shot/test', feature_dir, device=device, model=model.model_v, audio_from=audio_from)  # 3-shot test
    # train_dataset = AVS('v3_5_shot/train', feature_dir, device=device, model=model.model_v, audio_from=audio_from)  # 5-shot train
    # train_dataset = AVS('v3_6_shot/test', feature_dir, device=device, model=model.model_v, audio_from=audio_from)  # 5-shot test
